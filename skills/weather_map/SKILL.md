---
name: weather_map
description: "当用户需要生成气象色斑图、温度分布图、降水分布图、气象热力图、天气可视化图时使用本技能。注意：如果用户使用了相对时间表达（如最近三天、近一周、近一个月），请改用 weather_map_recent 技能，本技能仅适用于用户明确给出具体日期的场景。本技能支持：平均温度、最高温度、最低温度、降水量。调用前必须确认参数完整：地区、气象要素、开始时间、结束时间。如果用户缺少必要参数，必须主动向用户询问缺失参数。"
keywords:
  - 气象
  - 色斑图
  - 温度
